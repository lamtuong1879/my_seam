...
def resize_image(input_image_path, output_height, output_width):
    ...
    if np.absolute(delta_row) > 0:
        ...
        delta_col = delta_row
    #for use later
    cv2.imwrite('./output/seams/seam_overlay.png', input_image)
    ...
    cv2.imwrite('./output/maps/energy_color.png', color_img)
    #optional, print energy map
    cv2.imwrite('./output/maps/energy_map.png', energy_map)
    output_image_name = ''
    ...
    if delta_col > 0:
        ...
        for dummy in range(delta_col):
            ...
            write_seam(seam, './output/seams/seam_overlay.png')
...
if __name__ == '__main__':
    image_input = 'download2.jpeg'
    path = './' + image_input
    img = cv2.imread(path)
    # print("Current dimensions (HxW): " + str(img.size))
    output_height = 100
    output_width = 120
    #resize image
