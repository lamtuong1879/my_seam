...
def resize_image(input_image_path, output_height, output_width):
    ...
    old_height, old_width = input_image.size[: 2]
    ...
    if np.absolute(delta_row) > 0:
        ...
        delta_col = delta_row
    #for use later
    # cv2.imwrite('./seams/seam_overlay.png', input_image)
    imsave('./tempDir/seam_overlay.png', input_image)
    ...
    imsave('./output/energy_color.png', input_image)
    #optional, print energy map
    # cv2.imwrite('images/output/maps/energy_map.png', energy_map)
    imsave('./output/maps/energy_map.png', energy_map)
    output_image_name = ''
    ...
    if delta_col > 0:
        ...
        for dummy in range(delta_col):
            ...
            write_seam(seam, './output/seams_overlay/seam_overlay.png')
        ...
        # save_final_img(output_image)
        imsave('./output/final/carved_output.png', output_image)
    return output_image
...
def save_final_img(img, name):
    imsave('./output/final/' + name + '_output.png', img.astype(np.uint8))
